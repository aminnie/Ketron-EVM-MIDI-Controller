...
class MIDIHandler:
    def __init__(self, midi_instance):
        ...
        self.manufacturer_id = bytearray([0x43, 0x7E])

        # Pre-allocate bytearrays for memory efficiency
        self.section_sysex = bytearray([0x00, 0x00, 0x00])
        self.tempo_sysex = bytearray([0x01, 0x00, 0x00, 0x00, 0x00, 0x00])    # Tempo 1 to 4
        self.cur_volume = 100
    ...
    def send_section_sysex(self, midi_value):
        ...
        try:
            ...
            self.section_sysex[1] = midi_value
            self.section_sysex[2] = MIDIStatus.ON
            sysex_message = SystemExclusive(self.manufacturer_id, self.section_sysex)
            self.midi.send(sysex_message)

            self.section_sysex[2] = MIDIStatus.OFF
            sysex_message = SystemExclusive(self.manufacturer_id, self.section_sysex)
            self.midi.send(sysex_message)
            return True
    ...
    def send_tempo_sysex(self, midi_value):
        """Send Tempo SysEx command"""
        try:
            # Send ON followed by OFF Message
            self.tempo_sysex[1] = midi_value
            self.tempo_sysex[5] = MIDIStatus.ON
            sysex_message = SystemExclusive(self.manufacturer_id, self.tempo_sysex)
            self.midi.send(sysex_message)

            self.tempo_sysex[5] = MIDIStatus.OFF
            sysex_message = SystemExclusive(self.manufacturer_id, self.tempo_sysex)
            self.midi.send(sysex_message)
            return True
        except Exception as e:
...
class KeyLookupCache:
    def __init__(self, config):
        ...
        self.macropad_key_map = [
            "1:Intro 1", "0:Main A", "0:Ending 1", 
            "0:Intro 2", "0:Main B", "0:Ending 2",
            "0:Intro 3", "0:Main C", "0:Ending 3", 
            "0:Intro 4", "0:Main D", "0:Break"
        ]
...
class StateManager:
    ...
    def check_timeouts(self):
        ...
        # Revert eotor to tempo after timeout
...
class EVMController:
    ...
    def _process_tempo(self, direction):
        ...
        if direction == 1:
            midi_value = self.key_cache.tempo_midis["Tempo Up"]
            self.display.update_text(3, "KNOB: Tempo Up{}".format(sign))
        else:
            sign = "-" if self.state.encoder_sign else ""
            midi_value = self.key_cache.tempo_midis["Tempo Down"]
            self.display.update_text(3, "KNOB: Tempo Down{}".format(sign))

        self.midi_handler.send_tempo_sysex(midi_value)
    ...
    def _update_display(self):
        ...
        if timeout_type == "timeout_rotor" or timeout_type == "timeout_volume":
            self.display.update_text(3, "KNOB: -")
            self.display.update_text(6, "KNOB MODE: Tempo")
            self._preset_pixels()
